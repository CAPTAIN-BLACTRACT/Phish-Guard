...
import { BADGES, MODULES, XP_PER_LEVEL } from '../../constants';
...
export function ProfilePage({ showToast }) {
    ...
    const rankTitle = level >= 10 ? "Grandmaster" : level >= 7 ? "Veteran" : level >= 4 ? "Guardian" : "Recruit";
    ...
    const MAX_LEVEL = 10;
    const curXP = XP_PER_LEVEL[level - 1] || 0;
    const nextXP = XP_PER_LEVEL[level] || curXP;
    const progress = level >= MAX_LEVEL
        ? 100
        : Math.min(100, Math.max(0, ((xp - curXP) / (nextXP - curXP)) * 100));
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 30 }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                        <div style={{ ...T.card, padding: 30, textAlign: "center" }}>
                            ...
                            <div style={{ color: "#00f5ff", fontFamily: "Share Tech Mono", fontSize: "0.8rem", marginBottom: 8 }}>LVL {level} {rankTitle.toUpperCase()}</div>
                            ...
                            <div style={{ marginTop: 15 }}>
                                <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.6rem", fontFamily: "Share Tech Mono", color: "var(--txt2)", marginBottom: 4 }}>
                                    <span>{level >= MAX_LEVEL ? "LEVEL CAP REACHED" : `LEVEL ${level} → ${level + 1}`}</span>
                                ...
                                <div style={{ fontSize: "0.6rem", color: "rgba(255,255,255,0.3)", marginTop: 3, textAlign: "right", fontFamily: "Share Tech Mono" }}>{level >= MAX_LEVEL ? `${xp} XP` : `${xp} / ${nextXP} XP`}</div>
                            </div>
                        </div>
